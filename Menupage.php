...
class Menupage
{
   ...
   public function create()
   {
      ...
      $menuIng1 = trim($this->request->getParameter('menu-item-ingredient1'));
      $menuIng2 = trim($this->request->getParameter('menu-item-ingredient2'));
      $menuIng3 = trim($this->request->getParameter('menu-item-ingredient3'));
      ...
      if (is_null($menuName) || strlen($menuName) == 0 ||
          ...
          ((is_null($menuIng1) || strlen($menuIng1) == 0) &&
           (is_null($menuIng2) || strlen($menuIng2) == 0) &&
           (is_null($menuIng3) || strlen($menuIng3) == 0)) ||
          !is_numeric($menuQty)) {
         throw new InvalidArgumentException("required form input missing. Name, Category, Price, Quantity, Description, and Ingredients must be valid.");
      }
      ...
      if (!empty($deletedMenuQueryResult)) {
         ...
         if (!(is_null($menuIng1) || strlen($menuIng1) == 0)) {
          $addMadeOfQueryStr = "INSERT INTO MadeOf " .
                              "(menuItem_name, ingredient_name) " .
                              "VALUES " .
                              "('$menuName', '$menuIng1' )";
         $addMadeOf = $this->dbProvider->insertQuery($addMadeOfQueryStr);
       }
         if (!(is_null($menuIng2) || strlen($menuIng2) == 0)) {
          $addMadeOfQueryStr = "INSERT INTO MadeOf " .
                              "(menuItem_name, ingredient_name) " .
                              "VALUES " .
                              "('$menuName', '$menuIng2' )";
        ...
        if (!(is_null($menuIng3) || strlen($menuIng3) == 0)) {
          $addMadeOfQueryStr = "INSERT INTO MadeOf " .
                              "(menuItem_name, ingredient_name) " .
                              "VALUES " .
                              "('$menuName', '$menuIng3' )";
